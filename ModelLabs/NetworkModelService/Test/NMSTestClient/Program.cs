...
namespace FTN.Services.NetworkModelService.TestClient
{
	public class Program
	{
			...
			try
			{
				...
				do
				{
                    ...
                    else if (str == "5")
                    {
                        try
                        {
                            Console.WriteLine("PowerTranformer");
                            ModelCode code = (ModelCode)0x1112000000030000;//Model Code za powerTranformere
                            tgda.GetExtentValuesPowerTranformer(code);//
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("PowerTranformer failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
                    else if (str == "6")
                    {
                        try
                        {
                            ModelCode code = (ModelCode)0x1112000000030000;
                            List<long> gids = tgda.GetExtentValues(code);

                            long gid = gids.Min();//Najmanji gid
                            Association aso = new Association()
                            {
                                PropertyId = (ModelCode)0x1112000000030319,//ovde ubacujem da mi treba iz PowerTranformera lista Windinga to je taj id
                                Type = (ModelCode.POWERTRWINDING),//i kazem da mi trebaju samo TransformerWinding
                                Inverse = false
                            };
                            tgda.GetRelatedValuesTransformer(gid, aso);
                        }
                        catch (Exception ex)
                        {
                            message = string.Format("TransformerWindings failed. {0}", ex.Message);
                            Console.WriteLine(message);
                            CommonTrace.WriteTrace(CommonTrace.TraceError, message);
                        }
                    }
		...
		private static void PrintMenu()
		{
			...
			Console.WriteLine("\t5) Get PowerTransformer values");
			Console.WriteLine("\t6) Get TranformerWindings values");

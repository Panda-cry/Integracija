...
namespace FTN.ESI.SIMES.CIM.CIMAdapter.Importer
{
	...
	public class PowerTransformerImporter
	{
		...
		private ResourceDescription CreateTransformerWinidngResourceDescription(FTN.TransformerWinding cimTransformerWinding)
		{
			ResourceDescription rd = null;
			if (cimTransformerWinding != null)
			{
				...
				////populate ResourceDescription
				PowerTransformerConverter.PopulateTransformerWindingsProperties(cimTransformerWinding, rd, importHelper, report);
			}
			return rd;
		}
		//// TODO:
		...
		{
			SortedDictionary<string, object> cimTransformerWinidngs = concreteModel.GetAllObjectsOfType("FTN.TransformerWinding");
			if (cimTransformerWinidngs != null)
			{
				foreach (KeyValuePair<string, object> cimPowerTransformerPair in cimTransformerWinidngs)
				{
					FTN.TransformerWinding cimTransformerWinidng = cimPowerTransformerPair.Value as FTN.TransformerWinding;

					ResourceDescription rd = CreateTransformerWinidngResourceDescription(cimTransformerWinidng);
					if (rd != null)
					{
						delta.AddDeltaOperation(DeltaOpType.Insert, rd, true);
						report.Report.Append("TransformerWinding ID = ").Append(cimTransformerWinidng.ID).Append(" SUCCESSFULLY converted to GID = ").AppendLine(rd.Id.ToString());
					}
					else
					{
						report.Report.Append("TransformerWinding ID = ").Append(cimTransformerWinidng.ID).AppendLine(" FAILED to be converted");
					}
				}
				report.Report.AppendLine();
			}
		}
		private ResourceDescription CreateTransformerWinidngTestResourceDescription(FTN.WindingTest cimWindingTest)
		{
			ResourceDescription rd = null;
		...
		private void ImportWindingTests()
		{
			...
			{
				foreach (KeyValuePair<string, object> cimPowerTransformerPair in cimWindingTests)
				{
					FTN.WindingTest cimWindingTest = cimPowerTransformerPair.Value as FTN.WindingTest;

					ResourceDescription rd = CreateTransformerWinidngTestResourceDescription(cimWindingTest);
					if (rd != null)
					{
						delta.AddDeltaOperation(DeltaOpType.Insert, rd, true);
